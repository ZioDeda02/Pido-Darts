...
settings.theme = normalize(settings.theme, ['green', 'blue', 'red', 'purple', 'light', 'custom'], defaults.theme);
settings.customAccent = normalizeColor(settings.customAccent, defaults.customAccent);
settings.customBg = normalizeColor(settings.customBg, defaults.customBg);
settings.customText = normalizeColor(settings.customText, defaults.customText);
settings.backgroundMode = normalize(settings.backgroundMode, ['image', 'gradient', 'solid', 'custom'], defaults.backgroundMode);
settings.customBgImage = typeof settings.customBgImage === 'string' ? settings.customBgImage : '';
settings.mobileLayout = normalize(settings.mobileLayout, ['app', 'site'], defaults.mobileLayout);
settings.navBehavior = normalize(settings.navBehavior, ['auto', 'always', 'manual'], defaults.navBehavior);
settings.uiScale = normalize(settings.uiScale, ['compact', 'normal', 'large'], defaults.uiScale);
settings.highContrast = Boolean(settings.highContrast);
settings.animations = settings.animations !== false;
settings.bgIntensity = Math.max(0, Math.min(70, Number(settings.bgIntensity) || defaults.bgIntensity));
...
if (settings.customBgImage) root.style.setProperty('--custom-bg-image', `url("${settings.customBgImage.replace(/"/g, '%22')}")`);
...
window.DartsAppearance = { defaults, load, save, apply };
})();
...
if (dataEnd > bytes.length) throw new Error('Archivio ZIP danneggiato.');
...
{ name: 'LEGGIMI.txt', text: 'Backup Pido Darts v1.0.4. Per ripristinarlo usa Impostazioni > Dati e backup > Importa backup.\n' }
];
...
const info = parseJsonFile(files, 'backup-info.json', null);
if (!info || info.app !== 'Pido Darts') throw new Error('Questo file non sembra un backup di Pido Darts.');
if (Number(info.formatVersion) > FORMAT_VERSION) throw new Error('Il backup è stato creato da una versione più recente e non è compatibile.');
const result = {
info,
...
['players', 'gameSessions', 'records', 'completedGames', 'idealRecords'].forEach(key => {
...
for (const player of validIdItems(backup.players)) await db.savePlayer(player);
for (const session of validIdItems(backup.gameSessions)) await db.saveGameSession(session);
for (const record of validIdItems(backup.records)) await db.saveRecord(record);
for (const game of validIdItems(backup.completedGames)) await db.saveCompletedGame(game);
for (const ideal of validIdItems(backup.idealRecords)) await db.saveIdealRecord(ideal);
if (backup.settings && typeof backup.settings === 'object') appearance.save({ ...appearance.defaults, ...backup.settings });
return;
}
...
if (!file) return;
try {
if (file.size > 30 * 1024 * 1024) throw new Error('Il backup supera il limite di 30 MB.');
...
window.PidoDartsUpdater = { version: APP_VERSION, checkForUpdates, compareVersions };
})();
...
const color = player.color || '#20d868';
...
element.textContent = '';
if (player.avatarType === 'image' && player.avatarValue) {
element.classList.add('has-image');
element.style.backgroundImage = `url(${player.avatarValue})`;
element.setAttribute('aria-label', `Avatar di ${player.name || 'giocatore'}`);
return;
}
const span = document.createElement('span');
span.textContent = player.avatarType === 'emoji' ? (player.avatarValue || '🎯') : initials(player.name);
...
card.style.setProperty('--player-color', player.color || '#20d868');
...
pill.style.setProperty('--player-color', player.color || '#20d868');
...
state.avatarMode = player.avatarType || 'initials';
state.emoji = player.avatarType === 'emoji' ? (player.avatarValue || '🎯') : '🎯';
state.imageData = player.avatarType === 'image' ? (player.avatarValue || '') : '';
state.color = player.color || '#20d868';
...
console.error(error);
profilesGrid.innerHTML = '<div class="database-error">Impossibile aprire l’archivio locale dei giocatori. Prova ad aprire Pido Darts in un browser moderno o tramite localhost.</div>';
...
element.style.backgroundImage = '';
element.classList.remove('has-image');
element.style.setProperty('--player-color', player?.color || '#20d868');
if (player?.avatarType === 'image' && player.avatarValue) {
element.classList.add('has-image');
element.style.backgroundImage = `url(${player.avatarValue})`;
return;
...
scoreFilter.innerHTML = '<option value="all">Tutti</option>';
...
best.innerHTML = `<small>BEST</small>${stats.bestTurn || '—'}`;
...
meta.innerHTML = `<strong>${winner ? `🏆 ${winner.player?.name || 'Vincitore'}` : 'Completata'}</strong><small>${formatDate(game.completedAt)}</small><b>›</b>`;
...
card.style.setProperty('--player-color', participant.player?.color || '#20d868');
...
el.style.setProperty('--player-color', player?.color || '#20d868');
if (player?.avatarType === 'image' && player.avatarValue) {
el.classList.add('has-image');
el.style.backgroundImage = `url(${player.avatarValue})`;
return;
}
const span = document.createElement('span');
span.textContent = player?.avatarType === 'emoji' ? (player.avatarValue || '🎯') : initials(player?.name);
...
button.style.setProperty('--player-color', player.color || '#20d868');
button.style.setProperty('--player-text-color', textColorFor(player.color || '#20d868'));
...
row.style.setProperty('--player-color', player.color || '#20d868');
...
? 'PARTITA · MULTIPLAYER v1.0.4'
: (isComputerMatch()
? `PARTITA · VS PIDO PC · ${computerDifficultyLabel(state.computerDifficulty).toUpperCase()}`
: (isIdealChallenge() ? 'PARTITA · RECORD IDEALE v1.0.4' : (isRecordChallenge() ? 'PARTITA · BATTI IL RECORD v1.0.4' : 'PARTITA · ALLENAMENTO v1.0.4')));
}
gamePlayerCard?.style.setProperty('--player-color', player.color || '#20d868');
...
row.style.setProperty('--player-color', participant.player.color || '#20d868');
